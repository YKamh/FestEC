...
public class TabPagerAdapter extends FragmentStatePagerAdapter {
    ...
    public TabPagerAdapter(FragmentManager fm, JSONObject data) {
        ...
        for (int i = 0; i< size; i++){
            ...
            final ArrayList<String> eachTabPictruesArray = new ArrayList<>();
            final int pictureArraySize = pictureUrls.size();
            for (int j = 0; j < pictureArraySize; j++){
                eachTabPictruesArray.add(pictureUrls.getString(j));
            }
            TAB_TITLES.add(name);
            PICTURES.add(eachTabPictruesArray);
        }
    }
